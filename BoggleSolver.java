...
public class BoggleSolver
{
	...
	private boolean isValid(int row, int col, int direction){
		...
		if(direction ==0){ //upleft
			if(col==0||row==0){
				return false;
			}
			else{
				peacock = theBoard[row-1][col-1];
			}
		}

		if(direction ==2){// upright
			if(row==0||col==3){
				return false;
			}
			else{
		...
		boolean poopcock;
		poopcock = Character.isLowerCase(peacock);
		return poopcock;
